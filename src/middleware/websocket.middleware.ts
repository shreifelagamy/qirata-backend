...
class WebSocketMiddleware {
    ...
    async sessionOwner(socket: AuthenticatedSocket): Promise<MiddlewareResult> {
        try {
            ...
            if (!sessionId || !userId) {
                return {
                    ...
                    error: {
                        ...
                        code: 'MISSING_SESSION_ID_OR_USER_ID'
                    }
                };
            }
            ...
            if (!isOwner) {
                return {
                    success: false,
                    error: {
                        message: 'User is not the owner of the session',
            ...
            logger.error('Error in session owner middleware:', error);
            return {
                success: false,
                error: {
                    message: 'Session owner check failed',
                    code: 'SESSION_OWNER_ERROR'
                }
            };
        }
    }
    ...
    getMiddleware(name: string): (socket: AuthenticatedSocket, data: any, options?: any) => Promise<MiddlewareResult> {
        switch (name) {
            ...
            case 'sessionOwner':
                return this.sessionOwner.bind(this);
            default:
